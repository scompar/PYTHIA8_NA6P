...
//Reading external parameters
  for(int ig=0;ig<argc;++ig){
    ...
    if(str.Contains("--events")){
      sscanf(argv[ig+1],"%d",&nEvent);
    }
    else if(str.Contains("--energy")){
      sscanf(argv[ig+1],"%f",&energy);
    }
    else if(str.Contains("--ymin")){
      sscanf(argv[ig+1],"%f",&ymin);
    }
    else if(str.Contains("--ymax")){
      sscanf(argv[ig+1],"%f",&ymax);
    }
    else if(str.Contains("--particle")){
      sscanf(argv[ig+1],"%d",&specparticle);
    }
...
// Loop for event analysis    
    for (int i = 0; i < pythia.event.size(); ++i) {
      ...
      if (std::find(partons.begin(),partons.end(),std::abs(pythia.event[i].id())) != partons.end()) continue; // it is a parton, skip
      if (isDiquark(pythia.event[i].id())) continue; // it is a diquark, skip
      if (pythia.event[i].id()==90) continue; // it is the initial system, skip
      if (!pythia.event[i].isFinal()) continue;   // skip non-final particles
